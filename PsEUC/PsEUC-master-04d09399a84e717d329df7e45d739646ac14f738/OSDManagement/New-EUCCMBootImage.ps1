...
'@
    ...
    #Copy-Item -Path "c:\temp.wim" -Destination 'c:\temp'

    $result = Mount-WindowsImage -ImagePath "d:\sources\boot.wim" -Path "$WINPE_WORKING_FOLDER" -Index 1
    ...
    $result = Add-WindowsDriver -Recurse -Driver '\\m562prd1\src$\CM_Managed\Drivers\Boot-Critical' -Path "$WINPE_WORKING_FOLDER"

    robocopy 'D:\SMS\data' "$WINPE_WORKING_FOLDER\SMS\data"
